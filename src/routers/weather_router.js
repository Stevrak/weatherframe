...
// iframe call finds daily info but appends mock data
// for forcast (next 5 days have fake data generated by forecast_addon)
...
router.get('/weather_iframe/:location', async (req,res)=>{
  ...
  if (!req.params.location){
      lookup = getIp(req)
      //res.send("no location for weather iframe specified.");
  }else{
    lookup = req.params.location;
  }

  const data = await getGeoWeather(lookup).catch((e)=>{
      res.send('<div style="color:red">'+ e +'</div>')
  });

  // successful, return iframe compatible page?
  // again, with mock 5-day forecast tacked on
  if (data){
...
// if no provided location for ifram call
// look up user ip and use that

router.get('/weather_iframe', async (req,res)=>{
  // get ip of the user
  const ip = await getIp(req);

  const data = await getGeoWeather(ip).catch((e)=>{
      res.send('<div style="color:red">'+ e +'</div>')
  });
